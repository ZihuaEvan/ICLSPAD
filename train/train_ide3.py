...
parser.add_argument("--basepath", type=str, default="/seu_share/home/yangxu/230238542/ICLSD/ide3")
parser.add_argument(
    "--configpath",
    type=str,
    default="/seu_share/home/yangxu/230238542/ICLSD/EAGLE_EYE/eagle_eye/train/ide_config.json",
)
parser.add_argument("--lr", type=float, default=2e-5)
parser.add_argument("--bs", type=int, default=1)
parser.add_argument("--gradient-accumulation-steps", type=int, default=1)
parser.add_argument(
    "--tmpdir", type=str, default='/seu_share/home/yangxu/230238542/ICLSD/features_test'
)
parser.add_argument(
    "--outdir", type=str, default="./weights/ide3test"
)
parser.add_argument(
    "--cpdir", type=str, default="./weights/ide3test"
)
...
head = torch.nn.Linear(
    baseconfig.text_config.hidden_size, baseconfig.text_config.vocab_size if hasattr(baseconfig.text_config, "vocab_size") else baseconfig.vocab_size, bias=False
)
...
try:
    with open(os.path.join(args.basepath, "model.safetensors.index.json"), "r") as f:
        ...
        if "lm_head.weight" in index_json["weight_map"]:
            head_key = "lm_head.weight"
        elif "language_model.lm_head.weight" in index_json["weight_map"]:
            head_key = "language_model.lm_head.weight"
        else:
            raise ValueError(f"lm_head not found, available keys: {list(index_json['weight_map'].keys())[:20]}")

        head_path = index_json["weight_map"][head_key]
...
for epoch in range(num_epochs + 1):
    ...
    if accelerator.is_local_main_process:
        for id, i in enumerate(top_3acc):
            print({f"train/epochtop_{id + 1}_acc": i.sum().item() / total})
    if accelerator.is_local_main_process:
        print("Epoch [{}/{}], Loss: {:.4f}".format(epoch + 1, num_epochs, epoch_loss))
        print("Train Accuracy: {:.2f}%".format(100 * correct / total))
        print({"train/epochacc": correct / total, "train/epochloss": epoch_loss})
    ...
    if (epoch + 1) % train_config["save_freq"]:
        ...
        if accelerator.is_local_main_process:
            for id, i in enumerate(top_3acc):
                print({f"test/top_{id + 1}_acc": i.sum().item() / total})
        epoch_loss /= num_batches
        if accelerator.is_local_main_process:
            print(
            ...
            save_path = f"{args.cpdir}/model_epoch{epoch}.bin"
