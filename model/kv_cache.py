...
def initialize_past_key_values(model, max_seq_len=None):
    """
    Initialize dynamic KVCache for all layers of the model
    Args:
        model: transformer model with config attributes
        max_seq_len: initial maximum sequence length
    Returns:
        past_key_values: List[List[KVCache]]  # [layer][k/v]
        past_key_values_data_list: list of tensors per device
        current_length_data: torch.Tensor tracking length
    """
    ...
    for i in range(config.num_hidden_layers):
        try:
            device = model.text_model.layers[i].self_attn.q_proj.weight.device
        except:
            device = model.layers[i].self_attn.q_proj.weight.device
        devices.append(device)
    past_key_values_data_list = []
    # Prepare data per device
    startnum = 0
