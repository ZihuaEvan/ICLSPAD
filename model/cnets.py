...
class Model(nn.Module):
    def __init__(self, config, load_emb=False, path=None, bias=True):
        ...
        if load_emb:
            ...
            import json
            if Type == "Idefics3ForConditionalGeneration":
                safetensors_index = os.path.join(path, "model.safetensors.index.json")

                try:
                        with open(safetensors_index, "r") as f:
                            index_json = json.load(f)
                            emb_path = index_json["weight_map"].get("model.text_model.embed_tokens.weight", None)
                        if emb_path is not None:
                            with safe_open(os.path.join(path, emb_path), framework="pt", device="cpu") as f:
                                tensor_slice = f.get_slice("model.text_model.embed_tokens.weight")
                                vocab_size, hidden_dim = tensor_slice.get_shape()
                                tensor = tensor_slice[:, :hidden_dim].float()
                        else:
                            print("Warning: 'model.embed_tokens.weight' not found in safetensors index.")
                except:
                    print(f"Failed to load embedding from safetensors")
